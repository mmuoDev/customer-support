...
const { validate, verifyToken, checkCategoryExists, isCustomer, isAuthorized, isAdmin, checkStatusExists } = require('../middlewares/validators')
const httpResponse = require('../helpers/httpResponse');

routes.post('/',  checkCategoryExists, verifyToken,  isCustomer, validate,  async (req, res) => {
    try{
        const newTicket = await ticketService.add(req.body.subject, req.body.description, req.body.category, req.userId)
...
routes.post('/:id/comments',  verifyToken, isAuthorized, validate,  async (req, res) => {
    try{
        const newComment = await commentService.add(req.body.comment, req.params.id, req.userId)
...
routes.put('/:id/resolve',  verifyToken, isAdmin, checkStatusExists,  async (req, res) => {
    try{
        const ticket = await ticketService.resolve(req.params.id, req.body.status)
